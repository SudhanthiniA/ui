...
const {
  ...
  STATUS_RECONSIDER_REJECTED_SITE_VISIT, STATUS_PENDING_SITE_VISIT, STATUS_PENDING_DSC,
...
@Component({
  selector: 'app-distillery-license-view',
  templateUrl: './distillery-license-view.component.html',
  styleUrls: ['./distillery-license-view.component.scss']
})
export class DistilleryLicenseViewComponent implements OnInit {
  ...
  CheckListCodeList = CheckListCodeConstants.DISTILLERY;
  ...
  getSiteDocFormCntridx(idx) { return this.getSiteDocumentFormCntr().get(idx); }
  getSiteDocumentByIdxFormCntr(idx, element = null) {
  return (element) ? this.getSiteDocFormCntridx(idx).get(element) : this.getSiteDocFormCntridx(idx); }
  ...
  loadLicenseData() {
    this.licenseSer.findLicenseApplication(this.applicationNumber).subscribe((resp: any) => {
      ...
      if (respCode === 200) {
        ...
        const { status: licenseStatus, stage, siteDetails, commentList } = this.licenseData;
        ...
        this.showAppicantSiteVisitDet = this.customerSiteDetails;
        this.showOfficierSiteVisitDet = this.officierSiteDetails && this.isOfficier ;
  ...
  processFormActions() {
    ...
    this.licenseSer.findWorkFlowDetailsByLoginUserDesignation(MODULE_NAME_CODE, workFlowName).subscribe((resp: any) => {
      ...
      if (respCode === 200) {
        ...
        if (respCont) {
          const { level, isRejected, isSendBack, sendBackList, isForwardWithOutApproval, isFinished, isSiteVisitOfficer } = respCont;
          ...
          console.log('this.showSiteVisitForm::', this.showSiteVisitForm);
          /**
