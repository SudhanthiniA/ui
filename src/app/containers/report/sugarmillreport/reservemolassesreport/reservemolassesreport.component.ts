...
@Component({
  selector: 'app-reservemolassesreport',
  templateUrl: './reservemolassesreport.component.html',
  styleUrls: ['./reservemolassesreport.component.scss']
})
export class ReservemolassesreportComponent implements OnInit {

  reservemolassesreport: any[] = [];
  ...
  getIndentRequest() {
    this.apiService.get('reservemolassesreport').then((responceData: any[]) => {
      console.log(responceData);
      this.reservemolassesreport = responceData;
  ...
  generatePdf() {
    ...
    body1.push([{}, { text: 'Rate Details of Reserve Molasses Report', alignment: 'center'}, {}]);
    ...
    // dynamic row push
    for (const key in this.reservemolassesreport) {
      if (this.reservemolassesreport.hasOwnProperty(key)) {
        const ipdayKey = this.reservemolassesreport[key];
    ...
    this.utilsSer.createPDF( 'Rate Details of Reserve Molasses Report', dateData );
    // pdfMake.createPdf(dateData).download('Rate Details of Reserve Molasses Report');
  }
  ...
  generateXls() {
    ...
    this.utilsSer.createXLSX( 'Rate Details of Reserve Molasses Report.xlsx', element );
    // const ws: XLSX.WorkSheet = XLSX.utils.table_to_sheet(element);
    // /* generate workbook and add the worksheet */
    // const wb: XLSX.WorkBook = XLSX.utils.book_new();
    // XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
    // /* save to file */
    // XLSX.writeFile(wb, 'Rate Details of Reserve Molasses Report.xlsx');
  }
