...
@Component({
  selector: 'app-insideupdistilleryforcheavy',
  templateUrl: './insideupdistilleryforcheavy.component.html',
  styleUrls: ['./insideupdistilleryforcheavy.component.scss']
})
export class InsideupdistilleryforcheavyComponent implements OnInit {

  insideupdistilleryforCheavyreport: any[] = [];
  ...
  getIndentRequest() {
    this.apiService.get('insideupdistilleryforC-heavyreport').then((responceData: any[]) => {
      console.log(responceData);
      this.insideupdistilleryforCheavyreport = responceData;
  ...
  generatePdf() {
    ...
    body1.push([{}, { text: 'Inside UP Distillery For C-Heavy ', alignment: 'center'}, {}]);
    ...
    // dynamic row push
    for (const key in this.insideupdistilleryforCheavyreport) {
      if (this.insideupdistilleryforCheavyreport.hasOwnProperty(key)) {
        const ipdayKey = this.insideupdistilleryforCheavyreport[key];
    ...
    this.utilsSer.createPDF( 'Inside UP Distillery For C-Heavy', dateData );
    // pdfMake.createPdf(dateData).download('Inside UP Distillery For C-Heavy');
  }
  ...
  generateXls() {
    ...
    this.utilsSer.createXLSX( 'Inside UP Distillery For C-Heavy Report.xlsx', element );
    // const ws: XLSX.WorkSheet = XLSX.utils.table_to_sheet(element);
    // /* generate workbook and add the worksheet */
    // const wb: XLSX.WorkBook = XLSX.utils.book_new();
    // XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
    // /* save to file */
    // XLSX.writeFile(wb, 'Inside UP Distillery For C-Heavy Report.xlsx');
  }
