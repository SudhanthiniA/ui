...
@Component({
  selector: 'app-adddispatch',
  templateUrl: './adddispatch.component.html',
  styleUrls: ['./adddispatch.component.scss']
})
export class AdddispatchComponent implements OnInit {
  ...
  getDetailsById(id){
    this.dispatchService.getDispatchById(id).subscribe((resp: any) => { 
      console.log(resp);
      if(resp.status== "s"){
  ...
  submit(status) {
      let postParams = {
        ...
        "status": status,
        "totBulkLitresScanned": this.dispatchObj.totBulkLitresScanned,
        "totScannedBottles": this.dispatchObj.totScannedBottles,
        ...
        "transportPassDetails": {
          ...
          "unitLicenseType":"BWFL",
          "unitName":'wholesale',
          "unitAddress":'Plot no :12, Vaibhav nagar,\n Lucknow, UP -600001',
        },
      }
      if(status ==="SUBMITTED"){
         if(this.totCasesScanned>0){
          this.dispatchService.addDispatch(postParams).subscribe((resp: any) => {
            if(resp.status==="s"){
              this.alert.showSuccess(resp.userDisplayMesg, 'Success');
              this.router.navigate(['/supplychainmanagement/warehouse/dispatch/list'])
  ...
  casesBottlesCalculation(data){
    ...
    let totBulkLitres=0;
    let indentDatas=[];
    indentDatas=data.indentOrderPickUpDetailsLst
