...
@Component({
  selector: 'app-vieworderallocation',
  templateUrl: './vieworderallocation.component.html',
  styleUrls: ['./vieworderallocation.component.scss']
})
export class VieworderallocationComponent implements OnInit {
    ...
    if (orderallocationForm.valid === false) {
      ...
      if (this.editId) {
        ...
        this.apiService.patch(url, postParam).then(responceData => {
          this.router.navigate(['/supplychainmanagement/distillery/orderallocation/list']);
        }).catch((err: any) => {
          console.log('\n err...', err);
        });
      } else {
