...
@Component({
  selector: 'app-addeditsampledetailsentry',
  templateUrl: './addeditsampledetailsentry.component.html',
  styleUrls: ['./addeditsampledetailsentry.component.scss']
})
export class AddeditsampledetailsentryComponent implements OnInit {
    ...
    if (sampleDetailForm.valid === false) {
      ...
      if (this.editId) {
        ...
        this.apiService.patch(url, postParam).then(responceData => {
          this.router.navigate(['/supplychainmanagement/distillery/sampledetailsentry/list']);
        }).catch((err: any) => {
          console.log('\n err...', err);
        });
      } else {
